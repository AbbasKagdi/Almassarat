...
if (in_array($_POST['p'], $pass)) {
	...
	if($init = file_get_contents("static/blogs/$brand/log.json")){
		...
		// update log
		file_put_contents("static/blogs/$brand/log.json", json_encode($text));
		// open post file
		$blog['path'] = "static/blogs/$brand/$id.json";
	...
	function revert($f, $bak, $pat, $msg, $brand){
		fclose($f);
		file_put_contents("static/blogs/$brand/log.json", $bak, LOCK_EX);
		unlink($pat);
		die($msg);
	}
	...
	if(empty($_FILES['file'])){
		revert($blog['file'], $init, $blog['path'], "Broken image upload", $brand);
	}
	...
	if(!in_array($extension, $extensions)){
		revert($blog['file'], $init, $blog['path'], "Uploaded file is not an image!", $brand);
	}
	...
	if($_FILES['file']['size'] < 5242880){
		try{
			move_uploaded_file($_FILES['file']['tmp_name'],"img/blogs/$brand/". $id .'.'. $extension);
		}
		catch(Exception $e){
			revert($blog['file'], $init, $blog['path'], $e, $brand);
		}
	}
	else{
		revert($blog['file'], $init, $blog['path'], "File size must be less than 5 MB", $brand);
	}
