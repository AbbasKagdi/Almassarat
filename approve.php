<?php
    ...
    if(!(isset($_SESSION['create'])) || !(isset($_SESSION['approve']))){
        if(!(isset($_GET['user'])) || !(isset($_GET['id'])) || !(isset($_GET['action']))){
            ...
            if(!(is_numeric($short_id)) || !(is_numeric($brand)))
                banish();

            // brand 1 or 0
            if(in_array($brand, array(1,0)))
...
<html lang="en">
  <head>
    <title>Approve a blog | Almassarat</title>
	<meta name="description" content="Approve a blog | Almassarat" />
    <!-- Load Headers-->
...
<table class="table table-border table-striped table-hover text-center">
    ...
    <tbody>
    <tr>
        <td>TEMP-100000001</td>
        <td><a href="<?php echo $brand; ?>-feed.php?article=-100000001" target="_blank" class="btn btn-info">Review</a></td>
        <td><input type="submit" class="btn btn-success" value="Approve" disabled></td>
        <td><a href="static/blogs/<?php echo $brand; ?>/100000001.json" class="btn btn-primary" download>Download</a></td>
    ...
    for($i = 1; $i < $log['pending_total']; $i++){
        ...
        <tr>
            ...
            <td><a href='static/blogs/$brand/$id.json' class='btn btn-primary' download>Download</a></td>
