<?php
    ...
    // load news log
    $log = json_decode(file_get_contents("static/blogs/grt/log.json"), true);
...
<html lang="en">
  <head>
    <title>GRT News | Almassarat</title>
    <meta name="description" content="GRT News Gallery." />
    <meta name="keywords" content="GRT MENA, Global Road Technology, GRT News">
    <!-- Load Headers -->
...
<div class="jumbotron jumbotron-fluid pb-0 bg-white" id="intro">
  ...
  <hr width="40%" class="hr-orange mt-5">
</div>
...
for($j = $start; $j < $stop; $j++){
    // fetch blog
    $blog = json_decode(file_get_contents("static/blogs/grt/". $log['posts'][$j] .".json"), true);
    // save memory
    unset($blog['content']);
    // collect badges
    $badges = [];
    ...
    $row1
        <div class='col-md-4'>
            <a href='grt-feed.php?article=".urlencode($blog['subtitle'])."-".$blog['bid']."' target='_blank'>
                <div class='card mb-4 shadow5' style='border: 1px solid #ec7404'>
                    <img class='card-img-top' src='img/blogs/grt/".$blog['image']."' alt='".$blog['title']."'>
                    <div class='card-body'>
...
<!-- Pagination -->
    ...
    <nav>
        <ul class="pagination justify-content-center mx-5">
            <li class="page-item">
                <a class="page-link" <?php  if($page<=1){ echo $disable_link; } ?> href='grt-news.php?page=<?php echo ($page-1); ?>' aria-label='Previous'><span aria-hidden='true'>&laquo;</span></a>
            </li>
            <li class="page-item">
                <a class="page-link" href='grt-news.php?page=1'>First</a>
            </li>
            <?php
            for($i=1; $i<=$pages; $i++){
                if($i < 4){
                    if($i==$page){
                        echo "<li class='page-item active'><a class='page-link' href='grt-news.php?page=$i'>$i</a></li>";
                    }
                    else{
                        echo "<li class='page-item'><a class='page-link' href='grt-news.php?page=$i'>$i</a></li>";
                    }
                }
                else if($i == 4){
        ...
        ?>
            <li class="page-item">
                <a class="page-link" href='grt-news.php?page=<?php echo $pages; ?>'>Last</a>
            </li>
            <li class="page-item">
                <a class="page-link" <?php if($page>=$pages){ echo $disable_link; } ?> href='grt-news.php?page=<?php echo ($page+1); ?>' aria-label='Next'><span aria-hidden='true'>&raquo;</span></a>
            </li>
        </ul>
    </nav>
